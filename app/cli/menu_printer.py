...
_router: Dict[str, MenuFunctions] = {
    "0.1": MenuFunctions(
        executor=collector.login, navigator=lambda: constants.MAIN_MENU
    ),
    ...
    "1.4": MenuFunctions(
        executor=collector.get_transactions_by_user,
        printer=printer_utils.print_transactions_table,
    ),
    "2.1": MenuFunctions(
        executor=user_service.get_all_users, printer=printer_utils.print_users_table
    ),
    "2.2": MenuFunctions(
        executor=collector.create_user, printer=lambda x: _console.print(f"\n{x}")
    ),
    "2.3": MenuFunctions(
        executor=collector.delete_user, printer=lambda x: _console.print(f"\n{x}")
    ),
    "3.1": MenuFunctions(
        executor=portfolio_service.get_all_portfolios,
        printer=printer_utils.print_portfolios_table,
    ),
    "3.2": MenuFunctions(
        executor=collector.create_portfolio, printer=lambda x: _console.print(f"\n{x}")
    ),
    "3.3": MenuFunctions(
        executor=collector.delete_portfolio, printer=lambda x: _console.print(f"\n{x}")
    ),
    "3.4": MenuFunctions(
        executor=collector.get_portfolio_by_id,
        printer=printer_utils.print_portfolio_with_investments_table,
    ),
    "3.5": MenuFunctions(
        executor=collector.liquidate_investment,
        printer=lambda x: _console.print(f"\n{x}"),
    ),
    "3.6": MenuFunctions(
        executor=collector.get_transactions_by_portfolio_id,
        printer=printer_utils.print_transactions_table,
    ),
    "4.1": MenuFunctions(
        executor=security_service.get_all_securities,
        printer=printer_utils.print_securities_table,
    ),
    "4.2": MenuFunctions(
        executor=collector.place_purchase_order,
        printer=lambda x: _console.print(f"\n{x}"),
    ),
    "4.3": MenuFunctions(
        executor=collector.get_transactions_by_ticker,
        printer=printer_utils.print_transactions_table,
    ),
}

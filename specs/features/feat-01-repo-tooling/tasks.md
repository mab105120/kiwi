...
- [x] Wire `make install` to `uv sync` across the backend workspace
- [x] Wire `make lint` to run ruff + mypy across backend packages
- [x] Wire `make test` to run pytest for `identity`, `app-api`, `worker`

import pytest
from app.cli.cli_wrapper_funcs import (
...
def test_collect_inputs(monkeypatch):
    inputs = {"First Name": "first_name", "Last Name": "last_name", "Age": "age"}
    ...
    assert collected == {"first_name": "John", "last_name": "Doe", "age": "30"}
...
def test_delete_user(db_session, monkeypatch):
    ...
    username = "testuser"
    monkeypatch.setattr(
        "app.cli.cli_wrapper_funcs._collect_inputs", lambda _: {"username": username}
    )
    result = delete_user()
...
def test_create_portfolio(db_session, monkeypatch):
    ...
    inputs = {"name": "test_portfolio", "description": "test_portfolio_description"}
    monkeypatch.setattr("app.cli.cli_wrapper_funcs._collect_inputs", lambda _: inputs)
...
def test_get_portfolio_by_id(db_session, monkeypatch):
    ...
    portfolio_id = portfolio_service.create_portfolio(
        "test_portfolio", "test_description", user
    )
...
def test_liquidate_investment(db_session, monkeypatch):
    monkeypatch.setattr(
        "app.cli.session_state.get_logged_in_user",
        lambda: User(
            username="testuser", firstname="", lastname="", password="", balance=0.0
        ),
    )
    monkeypatch.setattr(
        "app.service.portfolio_service.liquidate_investment",
...
def test_place_purchase_order(monkeypatch):
    monkeypatch.setattr(
        "app.service.security_service.execute_purchase_order",
    ...
    with pytest.raises(security_service.SecurityException) as e:
        monkeypatch.setattr(
            "app.cli.cli_wrapper_funcs._collect_inputs", lambda _: inputs
        )
        place_purchase_order()
    assert "Invalid input. Please try again." in str(e.value)
...
def test_get_transactions_by_user(monkeypatch):
    ...
    monkeypatch.setattr("app.cli.session_state.get_logged_in_user", lambda: user)
    monkeypatch.setattr(
        "app.service.transaction_service.get_transactions_by_user",
        lambda username: ["transaction1", "transaction2"],
    )
...
def test_get_transactions_by_portfolio_id(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda portfolio_id: ["transactionA", "transactionB"],
    )
...
def test_get_transactions_by_ticker(monkeypatch):
    ...
    monkeypatch.setattr("app.cli.cli_wrapper_funcs._collect_inputs", lambda _: inputs)
    monkeypatch.setattr(
        "app.service.transaction_service.get_transactions_by_ticker",
        lambda ticker: ["transactionX", "transactionY"],
    )
...
def test_login(monkeypatch):
    ...
    monkeypatch.setattr("app.cli.cli_wrapper_funcs._collect_inputs", lambda _: inputs)
    monkeypatch.setattr(
        "app.service.login_service.login",

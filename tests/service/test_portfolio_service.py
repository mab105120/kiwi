...
def setup(db_session):
    ...
    portfolio2 = Portfolio(
        name="Portfolio 2", description="Second portfolio", user=user
    )
...
def test_get_portfolios_by_user_db_failure(db_session, monkeypatch):
    ...
    with pytest.raises(Exception) as e:
        portfolio_service.get_portfolios_by_user(
            User(
                username="testuser", firstname="", lastname="", balance=0.0, password=""
            )
        )
    assert "Failed to retrieve portfolios due to error: Database query error" in str(
        e.value
    )
...
def test_get_all_portfolios_db_failure(db_session, monkeypatch):
    ...
    assert (
        "Failed to retrieve portfolios due to error: Database connection error"
        in str(e.value)
    )
...
def test_get_portfolio_by_id_db_failure(db_session, monkeypatch):
    ...
    assert (
        "Failed to retrieve portfolio due to error: Database connection error"
        in str(e.value)
    )
...
def test_create_portfolio_db_failure(db_session, monkeypatch):
    ...
    assert "Failed to create portfolio due to error: Database connection error" in str(
        e.value
    )
...
def test_delete_portfolio(setup, db_session):
    user = setup["user"]
    portfolio = Portfolio(
        name="To Be Deleted", description="This portfolio will be deleted", user=user
    )
    db_session.add(portfolio)
    db_session.commit()
    portfolio_service.delete_portfolio(portfolio.id)
    deleted_portfolio = (
        db_session.query(Portfolio).filter_by(id=portfolio.id).one_or_none()
    )
...
def test_liquidate_investment(setup, db_session):
    ...
    updated_investment = next(
        (inv for inv in portfolio.investments if inv.ticker == "AAPL"), None
    )
...
def test_liquidate_investment_insufficient_quantity(setup, db_session):
    ...
    assert (
        "Cannot liquidate 1000 shares of AAPL. Only 10 shares available in portfolio"
        in str(e.value)
    )

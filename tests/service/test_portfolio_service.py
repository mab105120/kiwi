from app.service.portfolio_service import _create_portfolio, add_investment_to_portfolio, build_portfolio_investments_table, build_portfolios_table, create_portfolio, delete_portfolio, get_all_portfolios, liquidate_investment
from app.session_state import reset_logged_in_user, set_logged_in_user
from app.domain import Investment, Portfolio, User
...
def test_build_portfolio_invalid_portfolio_id(monkeypatch):
...
def test_delete_portfolio_invalid_id(monkeypatch):
    monkeypatch.setattr("app.cli.input_collector.collect_inputs", lambda _: {"portfolio_id": "9999"})
    try:
        delete_portfolio()
    except Exception as e:
